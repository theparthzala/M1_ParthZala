#include "stdio.h"
#include "regist.h"
//#include"src/existreg.c"
//#include"src/regist.c"


// Program begins from this section
int main(){
    
    int check;         // A return parameter to check check of patient
    long pa;             // Variable that stores patient's aadhar number
    ...
    if(pd[j].vac_tp==1 && (vstat==1 || vstat==2))
        v1++;
    else if(pd[j].vac_tp==2 && (vstat==1 || vstat==2))
        v2++;
